...
import {
  Search, ArrowRight, ChevronRight, FileText, Grid2X2,
  ArrowLeft, Zap,
} from 'lucide-react';
...
// ─── Category icon map (reuses Lucide icons from constants) ──────────────────
  ...
  return (
    <Card className="hover:shadow-md transition-all hover:-translate-y-0.5 group">
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between gap-2">
          ...
          <Badge variant="outline" className="text-xs">
            {format.conversions.length} targets
          </Badge>
        </div>
      ...
      <CardContent className="space-y-3">
        {/* Target format badges */}
        <div className="flex flex-wrap gap-1.5">
          {targets.map((t) => (
            <Link
              key={t}
              href={`/convert?from=${format.name}&to=${t}`}
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground mt-1 max-w-2xl">
                    {meta?.description ?? `Convert between all ${meta?.name ?? data.title.toLowerCase()} formats online.`}
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-6 mt-4 text-sm">
                <div>
                  <span className="font-bold text-2xl">{data.formats.length}</span>
                  <span className="text-muted-foreground ml-1.5">source formats</span>
                </div>
                <div>
                  <span className="font-bold text-2xl">{totalPairs.toLocaleString()}</span>
                  <span className="text-muted-foreground ml-1.5">conversion pairs</span>
                </div>
                <div>
                  <span className="font-bold text-2xl text-green-600">Free</span>
                  <span className="text-muted-foreground ml-1.5">to start</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <Button asChild className="gap-2">
                <Link href={`/convert?category=${category}`}>
      ...
      <section className="py-12">
        <div className="container mx-auto px-4">
          ...
          <div className="relative max-w-lg mb-10">
            ...
            <Input
              type="search"
              placeholder={`Search ${data.formats.length} formats…`}
              className="pl-9 h-11"
              value={query}
          ...
          {query.trim() && (
            <p className="text-sm text-muted-foreground mb-6">
              {filtered.length === 0
                ...
                : `Showing ${filtered.length} of ${data.formats.length} formats`}
            </p>
          )}
      ...
      <section className="py-12 bg-muted/30 border-t">
        <div className="container mx-auto px-4">
          <h2 className="text-xl font-semibold mb-6">Other conversion categories</h2>
